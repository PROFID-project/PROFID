...
BASE <- "T:/PROFID/Study8"
DATADIR <- file.path(BASE, "Variable Selection & Model Development/Files")
OUTDIR <- file.path(BASE, "Time-to-event Analysis Framework 2/files_2")
...
# SUBSET: AF ONLY
# ----------------------------------------------------------
df_AF <- df[df$AF_group == "AF", ]

cat("AF-only sample size:", nrow(df_AF), "\n")

# ----------------------------------------------------------
# DEFINE FINAL VARIABLE SET (CLINICALLY RELEVANT)
# ----------------------------------------------------------
mandatory_vars <- c("Age", "LVEF", "Diabetes", "eGFR","BMI")
biomarkers <- c("Haemoglobin", "Cholesterol", "HDL", "LDL", "Triglycerides")
ecg_vars <- c("LBBB", "RBBB")
clinical_vars <- c("HF", "Hypertension")

all_vars <- c(mandatory_vars, biomarkers, ecg_vars, clinical_vars)

# ----------------------------------------------------------
# PREPARE DATA FOR MODEL
# ----------------------------------------------------------
df_AF_model <- df_AF[, c("Survival_time", "event_SCD", all_vars)]

# Remove rows with missing values (important for Cox)
df_AF_model <- na.omit(df_AF_model)

cat("Final sample used in model:", nrow(df_AF_model), "\n")
...
# ----------------------------------------------------------
cat("\n================ COX AF-ONLY SUMMARY ================\n")
print(summary(cox_AF))

# ----------------------------------------------------------
# SAVE COX RESULTS (HR, CI, p-values)
# ----------------------------------------------------------
cox_results <- broom::tidy(cox_AF, exponentiate = TRUE, conf.int = TRUE)

write.csv(
  cox_results,
  file.path(OUTDIR, "Cox_AF_only_selected_variables.csv"),
  row.names = FALSE
)

cat("✔ Cox model results saved\n")

# ----------------------------------------------------------
# PROPORTIONAL HAZARDS TEST
# ----------------------------------------------------------
cox_zph <- cox.zph(cox_AF)

cat("\n================ PH TEST ================\n")
print(cox_zph)

write.csv(
  as.data.frame(cox_zph$table),
  file.path(OUTDIR, "Schoenfeld_AF_only_selected.csv")
)

# ----------------------------------------------------------
# SAVE SCHOENFELD PLOTS
# ----------------------------------------------------------
pdf(file.path(OUTDIR, "Schoenfeld_AF_only_selected.pdf"))
plot(cox_zph)
dev.off()

cat("✔ PH diagnostics saved\n")

# ----------------------------------------------------------
# SAVE MODEL INFO
# ----------------------------------------------------------
model_info <- data.frame(
  Sample_Size = nrow(df_AF_model),
  Num_Covariates = length(cox_AF$coefficients),
  Global_PH_p = cox_zph$table["GLOBAL", "p"]
)

write.csv(
  model_info,
  file.path(OUTDIR, "Model_Info_AF_only_selected.csv"),
  row.names = FALSE
)
