...
# Map for extra variables (ICD_status, Cancer, COPD_cat)
icd_col    <- "ICD_status"
cancer_col <- "Cancer"
COPD_col   <- "COPD"

# Covariates in extended model
vars_extended <- c(
  "Age","Sex",
...
for (nm in names(fp_specs)) {
  ...
  for (i in 1:imp0$m) {
    cat("Fitting", nm, " - imputation", i, "of", imp0$m, "\n")
    flush.console()
    
    # 1. Completed data from untouched mids
    dat_i <- complete(imp0, i)
    
    # 2. Sanity check: same number of rows as preimp
    stopifnot(nrow(dat_i) == nrow(preimp))
    
    # 3. Add ICD_status, Cancer, COPD_cat from preimp *by row index*
    ...
    # 4. Cause-specific status
    dat_i$Status_cs1 <- ifelse(dat_i$Status == 1, 1L, 0L)
    
    # 5. Fit model in this imputation
    fits_nm[[i]] <- coxph(fmla, data = dat_i, x = TRUE, y = TRUE)
    aic_nm[i]    <- AIC(fits_nm[[i]])
  }
}

## ---- Summarise AIC safely ----

# keep only entries that are numeric vectors with at least 1 element
valid_specs <- fp_aic_list[
...
fp_aic_summary <- do.call(
  rbind,
  lapply(names(valid_specs), function(nm) {
    vec <- valid_specs[[nm]]
    data.frame(
      spec      = nm,
...
aic_mat <- sapply(valid_specs, identity)
aic_df  <- data.frame(
