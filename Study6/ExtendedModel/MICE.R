...
impute_vars <- c(
  ...
  #  auxiliary variables (extended model)
  ...
  # "LBBB", "RBBB", "AF_atrial_flutter", "HasMRI",
   "Baseline_type", "CVD_risk_region", 
  # "Anti_diabetic", "Anti_diabetic_oral", "Anti_diabetic_insulin",
  "MI_type"
)

# Split into categorical/numerical as imputation works differently 
categorical_vars <- c(
  # Base confounders
  "Sex",
  "Diabetes", "Hypertension", "Smoking", "MI_history",
...
pred <- make.predictorMatrix(imp_data)s
...
# Enforce use of PMM for the lipids (won't stabilise otherwise)
meth[lipids] <- "pmm"

# Restrict lipid predictors (no cross-prediction among lipids, wasn't stabilising)
pred[lipids, ] <- 0
...
# look some key variables to check imputation not impacting mean/sd  
