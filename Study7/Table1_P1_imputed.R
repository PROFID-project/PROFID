...
files <- list(
  ICD              = "ICD_imputed.csv",
  NonICD_reduced   = "NonICD_reduced_imputed.csv",   # Non-ICD, LVEF <= 35%
  NonICD_preserved = "NonICD_preserved_imputed.csv"  # Non-ICD, LVEF > 35%
)
...
out_csv <- "Table1_Phase1_imputed.csv"
fwrite(tbl, out_csv)
...
gt_tbl <- tbl |>
  ...
  tab_header(title = md("**Table 1: Phase 1: Baseline characteristics after imputation**"),
...
gtsave(gt_tbl, "Table1_Phase1_imputed.html")
