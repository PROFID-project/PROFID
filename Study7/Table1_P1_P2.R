...
file_ICD  <- "ICD_filtered_with_coords.csv"
file_NR   <- "NonICD_reduced_filtered_with_coords.csv"
file_NP   <- "NonICD_preserved_filtered_with_coords.csv"
...
# -------------------------
make_gt_html_p1 <- function(dt_compact, title, subtitle, out_html) {
...
make_gt_html_p2 <- function(dt_compact, title, subtitle, out_html) {
  if (nrow(dt_compact) == 0) {
    message("No rows to render for ", title)
    return(NULL)
  }
  ...
  gt_tbl <- gt(dt_compact, groupname_col = "Group") %>%
    tab_header(title = md(glue::glue("**{title}**")), subtitle = md(subtitle)) %>%
    cols_label(Variable = md("**Variable**"), Category = md("**Statistic / Level**"),
               ICD = md(glue::glue("ICD patients<br/>(n = {as.integer(nrow(df_ICD_cmr))})")),
               NonICD_reduced = md(glue::glue("Non-ICD patients<br/>≤35% (n = {as.integer(nrow(df_NR_cmr))})")),
               NonICD_preserved = md(glue::glue("Non-ICD patients<br/>>35% (n = {as.integer(nrow(df_NP_cmr))})")),
               `Missing data n (%)` = md("**Missing data n (%)**")) %>%
    tab_style(style = cell_text(weight = "bold"), locations = cells_row_groups()) %>%
    tab_options(table.font.size = px(13), data_row.padding = px(4)) %>%
    cols_align(align = "center", columns = vars(ICD, NonICD_reduced, NonICD_preserved, `Missing data n (%)`))
...
html1 <- make_gt_html_p1(as.data.frame(table1_phase1_compact), "Table 1: Phase 1: Baseline characteristics",
                      "Excluding cardiac magnetic resonance imaging parameters", "Table1_Phase1.html")
html2 <- make_gt_html_p2(as.data.frame(table1_phase2_compact), "Table 1: Phase 2: Baseline characteristics with CMR (HasMRI == 1)",
